...
public class Rygel.ODID.MetadataExtractor: GLib.Object {
    ...
    public void extract (File file, string content_type) {
		...
		} else if (file.get_basename ().has_suffix (".item")) {
			KeyFile keyFile = new KeyFile();
			try {
				keyFile.load_from_file(file.get_path (),
									   KeyFileFlags.KEEP_COMMENTS |
									   KeyFileFlags.KEEP_TRANSLATIONS);
				this.extract_item_information (keyFile);
				message ("Item file %s read.", file.get_uri ());

			} catch (Error error) {
				warning ("Unable to read item file %s, Message: %s", file.get_uri (), error.message);
			}
		} else {
			this.extract_basic_information (file, null, null);
        }
	...
	private void extract_item_information (KeyFile keyFile) throws Error {
		string title = keyFile.get_string ("item", "title");
		string[] resourceGroups = keyFile.get_string_list ("item", "resources");

		foreach (string group in resourceGroups) {
			message ("Read group %s", group);
			if (keyFile.has_group (group)) {
				string uri = keyFile.get_string (group, "uri");
				message ("URI = %s", uri);
